...
function createHtml(tagName, elAttr, tagContent) {
  ...
  if (tagContent != undefined) {
    tagContent instanceof Element
      ? documentEl.append(tagContent)
      : (documentEl.innerHTML = tagContent);
  }
...
function generateUserDetail(user) {
  ...
  let login = createHtml(
    "a",
    { href: user.html_url, class: "login" },
    "@" + user.login
  );
  let statTable = createTable([
    ["Respos", "Followers", "Following"],
    [user.public_repos, user.followers, user.following],
  ]);
  let address = createHtml("p", { class: "address" }, user.location);
  let userUrl = createHtml(
    "a",
    { class: "blogUrl", href: user.blog },
    user.blog
  );
  let twitter = createHtml(
    "p",
    { class: "twitter", title: "Not Available" },
...
themeSwitch.addEventListener("click", (ev) => {
  ...
  ev.target.innerHTML == "dark"
    ? (ev.target.innerHTML = "light")
    : (ev.target.innerHTML = "dark");
});
